...
public class MapVisualizer {
    ...
    public String draw(Vector2d lowerLeft, Vector2d upperRight) {
        ...
        for (int i = upperRight.get_y() + 1; i >= lowerLeft.get_y() - 1; i--) {
            if (i == upperRight.get_y() + 1) {
                builder.append(drawHeader(lowerLeft, upperRight));
            }
            builder.append(String.format("%3d: ", i));
            for (int j = lowerLeft.get_x(); j <= upperRight.get_x() + 1; j++) {
                if (i < lowerLeft.get_y() || i > upperRight.get_y()) {
                    builder.append(drawFrame(j <= upperRight.get_x()));
                } else {
